...
## Setup and Usage
For this project, we focused on simulating magnetocapillary interactions in both confined and periodic geometries. For each of the scenarios, 2 Matlab functions and 2 Matlab scripts are provided to set up, simulate, and visualize the pattern formation which arise as a function of system parameters, packing fraction, and boundary geometry. Matlab R2022b was used for making the code, but it has been shown to run on Matlab R2019a and Matlab 2023b as well. The typical install time for Matlab, for the first time, is 30 minutes to an hour.
...
Anaconda (https://www.anaconda.com/) must be downloaded. There are detailed instructions on the linked website for installation depending on the operating system. Then, the jupyter notebook environment is used. All necessary packages are installed as part of the code provided, so there should not be any need for downloading of non built-in software. The code should run between 30 seconds - 5 minutes on a standard Macbook Pro.

For the animations to work of the pattern formation over time, you need may need to run (if you haven't already done so within the jupyter notebook environment): conda install -c conda-forge ffmpeg.
