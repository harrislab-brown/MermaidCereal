function dydt = motion_nbod_confine(t,th,that,avec,mass_disk,I,m,f_0,R,l_c,mu_0,confinement_radius)
...
for qq = 1:length(avec)
    dydt = [dydt ; that./(2*mass_disk).*Fx_trans(qq) ; that./(2*mass_disk).*Fy_trans(qq) ];
end